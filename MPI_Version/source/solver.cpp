...
//              {i,j,Get_neighbor(k,iz,1 )}};
  ...
  if(count(value_phase(Range::all(),i,j,k)>cutoff)==1)
  {
    value_conc2(Phase_of_grain(index_grain1(1,i,j,k)),i,j,k)=total_conc(i,j,k);
    return;
  }
  Array<double,1> temp_conc(value_conc2(Range::all(),i,j,k));
  Partitioning( total_conc(i,j,k),value_phase(Range::all(),i,j,k),temp_conc );
  //value_conc2(Range::all(),i,j,k)=temp_conc;
  ;
  return;
}

double phase_field::Driving_force(const int grainii,const int phaseii,const int grainjj,const int phasejj,const double concii,const double concjj) const
{
    double df=0.;
...
void phase_field::Sort_xyz(Array<double,1>& value,Array<int,1>& grain,const int N)
{
   for(int i=1;i<=N;i++){
